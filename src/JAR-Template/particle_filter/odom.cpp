...
void Odom::update_position(float ForwardTracker_position, float SidewaysTracker_position, float orientation_deg){
  ...
  float X_position_delta = local_polar_length*sin(global_polar_angle); 
  float Y_position_delta = local_polar_length*cos(global_polar_angle);

  //printf("X: %f\nFTracker: %f\nSTracker:%f\nY: %f\nAngle:%f\n\n", local_X_position,  Forward_delta, Sideways_delta, local_Y_position, orientation_delta_rad);

  X_position-=X_position_delta;
  Y_position-=Y_position_delta;

  chassis.pose.x = X_position;
  chassis.pose.y = Y_position;
  chassis.pose.theta = chassis.get_absolute_heading();
}

double vertical_dist(){
  return (VDistance.objectDistance(inches) - 0.25);
} 

double horizontal_dist(){
  return (HDistance.objectDistance(inches) + 6.25);
}
